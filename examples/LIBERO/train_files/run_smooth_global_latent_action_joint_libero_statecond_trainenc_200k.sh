#!/usr/bin/env bash
# Joint 200k LIBERO policy: smooth global latent world model + state-cond
# action head + DINO encoder finetuning (train_encoder=true).
#
# This is the ablation over the 88% smooth baseline: restore proprio state
# conditioning (use_state_cond=true, include_state=true, state-normalized
# schema) and joint encoder finetuning (train_encoder=true, backbone lr 1e-6).
set -euo pipefail
...
RUN_ID="${RUN_ID:-lewm_oft_libero_dinov3b_smooth_global384_statecond_trainenc_200k}"
