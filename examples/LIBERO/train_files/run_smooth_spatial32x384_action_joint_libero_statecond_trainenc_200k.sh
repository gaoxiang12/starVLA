#!/usr/bin/env bash
# Joint 200k LIBERO policy with 32 explicit 384-D spatial tokens per frame,
# normalized state conditioning, and DINOv3-B encoder finetuning.
set -euo pipefail
...
CONFIG="${CONFIG:-examples/LIBERO/train_files/starvla_smooth_spatial32x384_action_joint_libero_statecond_trainenc_200k.yaml}"
...
RUN_ID="${RUN_ID:-lewm_oft_libero_dinov3b_smooth_spatial32x384_statecond_trainenc_200k}"
...
exec "${ACCELERATE_BIN}" launch \
  ...
  --framework.world_model.smooth_spatial_token_dim 384 \
  --framework.world_model.smooth_latent_dim 384 \
