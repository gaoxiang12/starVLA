#!/usr/bin/env bash
# Train the joint reconstructive compact-latent world model on LIBERO.
set -euo pipefail
...
CONFIG="${CONFIG:-examples/LIBERO/train_files/starvla_reconstructive_latent_wm_libero.yaml}"
DATA_ROOT="${DATA_ROOT:-playground/Datasets/LEROBOT_LIBERO_DATA}"
DATA_MIX="${DATA_MIX:-libero_all_reconstructive_wm_l10_augmented}"
RUN_ROOT="${RUN_ROOT:-playground/Checkpoints}"
RUN_ID="${RUN_ID:-lewm_oft_libero_dinov3b_reconstructive_latent_joint_short12_50k}"
BASE_WM="${BASE_WM:-dinov3_weights/dinov3_vitb16_pretrain_lvd1689m-73cec8be.pth}"
CUDA_DEVS="${CUDA_DEVS:-4}"
