#!/usr/bin/env bash
# Ten-thousand-step continuation with a two-step autoregressive rollout loss,
# resuming the smooth spatial LIBERO latent world model from the 10k
# single-shot checkpoint.
set -euo pipefail
...
CONFIG="${CONFIG:-examples/LIBERO/train_files/starvla_smooth_spatial_latent_wm_libero_rollout2.yaml}"
DATA_ROOT="${DATA_ROOT:-playground/Datasets/LEROBOT_LIBERO_DATA}"
DATA_MIX="${DATA_MIX:-libero_all_smooth_latent_wm_l10_augmented_rollout}"
RUN_ROOT="${RUN_ROOT:-playground/Checkpoints}"
RUN_ID="${RUN_ID:-lewm_oft_libero_dinov3b_smooth_spatial_global384_short12_far8_rollout2_10k}"
