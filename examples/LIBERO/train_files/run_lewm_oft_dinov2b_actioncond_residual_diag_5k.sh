#!/usr/bin/env bash
# Isolate the value of action conditioning with a zero-output causal residual.
# The 160k action-free delta head and all representation/policy modules stay frozen.
...
RUN_ID="${RUN_ID:-lewm_oft_libero_dinov2b_actioncond_residual_diag_from160k_5k}"
STEPS="${STEPS:-5000}"
SAVE_INTERVAL="${SAVE_INTERVAL:-1000}"
BATCH="${BATCH:-8}"
LR="${LR:-3e-5}"
WARMUP="${WARMUP:-250}"
CUDA_DEVS="${CUDA_DEVS:-4,5,6,7}"
...
accelerate launch \
  ...
  --framework.world_model.delta_head_action_residual true \
  --framework.world_model.delta_action_residual_depth "${ACTION_DEPTH:-2}" \
  --framework.world_model.delta_action_diagnostics true \
  --framework.world_model.delta_action_diagnostics_interval "${DIAG_INTERVAL:-50}" \
